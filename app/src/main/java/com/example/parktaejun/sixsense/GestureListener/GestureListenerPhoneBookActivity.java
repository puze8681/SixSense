...
public class GestureListenerPhoneBookActivity extends GestureDetector.SimpleOnGestureListener implements View.OnTouchListener {
    Context mContext;
    GestureDetector gDetector;

    public GestureListenerPhoneBookActivity(Context context) {
        this.mContext = context;
    }

    public GestureListenerPhoneBookActivity(Context c, AttributeSet attrs) {
        super();
    }

    public GestureListenerPhoneBookActivity(Context context, GestureDetector gDetector) {
        if (gDetector == null){
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        if (Math.abs(e1.getX() - e2.getX()) < 250 && (e1.getY() - e2.getY() > 0)) {
            ...
            if(!PhoneBookActivity.getEarPlugMode()) PhoneBookActivity.vibrate(Output.makeVibe(2));
            ...
            smActivityIntent.putExtra("number", PhoneBookActivity.PBitems.get(PhoneBookActivity.position).getPhoneNum());
            mContext.startActivity(smActivityIntent);
        } else if ((e1.getX() - e2.getX() > 0) && (e1.getY() - e2.getY() > 0)) {
            //왼쪽 위 대각선 드래그
            Toast.makeText(mContext, "message sending activity", Toast.LENGTH_SHORT).show();
            Intent scActivityIntent = new Intent(mContext, SMSContentActivity.class);
            mContext.startActivity(scActivityIntent);
        } else {
    ...
    public boolean onDown(MotionEvent e) {
        ...
        if (PhoneBookActivity.getEarPlugMode()) {
            PhoneBookActivity.speakTTS(PhoneBookActivity.getInfo());
        } else {
            PhoneBookActivity.vibrate(Output.makeVibe(1));
        }
        return true;
    }
