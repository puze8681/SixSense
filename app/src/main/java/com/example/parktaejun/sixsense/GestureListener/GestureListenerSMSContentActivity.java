...
public class GestureListenerSMSContentActivity extends GestureDetector.SimpleOnGestureListener implements View.OnTouchListener {
    Context mContext;
    GestureDetector gDetector;

    public GestureListenerSMSContentActivity(Context context) {
        this.mContext = context;
    }

    public GestureListenerSMSContentActivity(Context c, AttributeSet attrs) {
        super();
    }

    public GestureListenerSMSContentActivity(Context context, GestureDetector gDetector) {
        if (gDetector == null){
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        if (Math.abs(e1.getX() - e2.getX()) < 250 && (e1.getY() - e2.getY() > 0)) {
            ...
            if(!SMSContentActivity.getEarPlugMode()) SMSContentActivity.vibrate(Output.makeVibe(2));
            ...
            SMSContentActivity.nextDisplay('b');
        } else if (Math.abs(e1.getY() - e2.getY()) < 250 && (e2.getX() - e1.getX() > 0)) {
            //오른쪽 드래그
            Toast.makeText(mContext, "RIGHT", Toast.LENGTH_SHORT).show();
            SMSContentActivity.nextDisplay('n');
            ...
            smActivityIntent.putExtra("number", SMSContentActivity.SCitems.get(SMSContentActivity.position).getPhoneNum());
            mContext.startActivity(smActivityIntent);
        } else if ((e1.getX() - e2.getX() > 0) && (e1.getY() - e2.getY() > 0)) {
            //왼쪽 위 대각선 드래그
            Toast.makeText(mContext, "message sending activity", Toast.LENGTH_SHORT).show();
            Intent pbActivityIntent = new Intent(mContext, PhoneBookActivity.class);
            mContext.startActivity(pbActivityIntent);
        } else {
    ...
    public boolean onDown(MotionEvent e) {
        ...
        if (SMSContentActivity.getEarPlugMode()) {
            SMSContentActivity.speakTTS(SMSContentActivity.getInfo());
        } else {
            SMSContentActivity.vibrate(Output.makeVibe(1));
        }
        return true;
    }
